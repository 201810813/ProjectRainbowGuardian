...
{
private:
	static	shared_ptr<RandomManager>	Instance;
	...
	RandomManager& operator = (const RandomManager& other) = delete;

	static void Destroy(RandomManager* InstancePtr)
	{
		delete InstancePtr;
	}
...
public:
	static shared_ptr<RandomManager> GetInstance()
	{
		if (nullptr == Instance)
		{
			Instance = shared_ptr<RandomManager>(new RandomManager(), Destroy);
		}

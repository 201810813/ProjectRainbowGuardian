...
void Scene6F::makeLayout() {
	...
	if (randomValue <= 60) { // 확률 60%
        setupBattleRoom(); // 전투 방 설정
    }
	else if (randomValue <= 80) { // 확률 20% (60% ~ 80%)
        setupAltarRoom(); // 제단 방 설정
    }
    else {
		setupShopRoom(); // 상점 방 설정, 확률 20% (80% ~ 100%)
    }
}

void Scene6F::setupBattleRoom() {
    // Map Layout
    ...
    WriteManager::GetInstance()->AddLine(FMessageParam(LAYOUT_TYPE::STORY, "😭 이곳은 남색 슬픔의 층입니다.", false, 0, TEXT_COLOR_TYPE::BLUE_INENSITY));
    WriteManager::GetInstance()->AddLine(FMessageParam(LAYOUT_TYPE::STORY, "👻 눈물 망령의 대성통곡이 귀를 찌릅니다.", false, 1, TEXT_COLOR_TYPE::WHITE));
...
void Scene6F::setupAltarRoom() {
    // Map Layout
    WriteManager::GetInstance()->MakeLayout(LAYOUT_TYPE::MAP, 104, 2, 9, 9);
    WriteManager::GetInstance()->AddLine(FMessageParam(LAYOUT_TYPE::MAP, "06 [⛲]", false, 3, TEXT_COLOR_TYPE::BLUE));
    // Story Layout
    WriteManager::GetInstance()->MakeLayout(LAYOUT_TYPE::STORY, 0, 12, 9, 60);
...
void Scene6F::setupShopRoom() {
    // Map Layout
    WriteManager::GetInstance()->MakeLayout(LAYOUT_TYPE::MAP, 104, 2, 9, 9);
    WriteManager::GetInstance()->AddLine(FMessageParam(LAYOUT_TYPE::MAP, "06 [🏪]", false, 3, TEXT_COLOR_TYPE::ORANGE));

...
- **추가 구현 사항**
  ...
  6. 레이아웃 시스템 구현 (+ 출력 시스템)
        - 콘솔창에 구역을 나누어 UI처럼 활용하기 위해 레이아웃 시스템을 구현했습니다.
  7. 더블 버퍼 구현
        - tick() 구현으로 인한 플리커링 현상을 해결하기 위해 더블 버퍼를 구현했습니다.
  9. TimeManager, PathManager, KeyManager, RandomManager, SceneManager, SoundManager 구현
        - 모든 Manager 객체들은 고유해야하기 때문에 싱글톤으로 구현했습니다.
        - TimeManager   : DT 계산을 위해 추가했습니다.
        - PathManager   : 사운드 파일 로드시 활용할 경로 관리자입니다.
        - KeyManager    : 키 입력을 tick()마다 체크해주는 관리자입니다.
        - RandomManager : 랜덤 값을 뽑아줄 관리자입니다.
        - SceneManager  : 여러 씬 전환, 현재 씬 지정, 등 씬에 관련한 정보를 관리해주는 관리자입니다.
        - SoundManager  : FMOD의 시스템 객체 초기화 등 사운드에 관련된 기능들을 관리해주는 관리자입니다.
  10. 애니메이션 구현
        - 더블 버퍼를 구현했기 때문에 이를 잘 보여줄 수 있는 애니메이션을 만들고자 했습니다.
        - 프레임 정보를 가지는 Animation과 이런 Animation을 관리해주는 Animator를 구현했습니다.
  12. 배틀 시스템 구현
        - 전투가 발생하는 씬의 부모인 MainScene에서 배틀시스템을 관리해주도록 해주었습니다.
        - 스테이트를 나누어 조건에 따라 다음 스테이트로 넘어가며 진행하도록 해주었습니다.
        - 저희는 턴제 게임을 만들었기 때문에 이런 배틀씬 관리가 필요했습니다.
  13. 커서 시스템 구현
